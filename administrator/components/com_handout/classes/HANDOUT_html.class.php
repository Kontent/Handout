...
class HandoutHTML extends JHTML
{
	...
	function categoryList($id, $action, $options = array())
	{
		...
		$parent = JHTML::_('select.genericlist', $options, 'catid', ' class="inputbox" size="1" onchange="' . $action . '"', 'value', 'text', $id, null, false,
				false);
		return $parent;
	}
	...
	function categoryParentList($id, $action, $options = array())
	{
		...
		// $cat->load($id);
		$cat = &HandoutCategory::getInstance2($id);
	...
	function imageList($name, &$active, $javascript = null, $directory = null)
	{
		...
		if (!$javascript) {
			$javascript = "onchange=\"javascript:if (document.adminForm." . $name
					. ".options[selectedIndex].value!='') {document.imagelib.src='../images/stories/' + document.adminForm." . $name
	...
	function uploadSelectList($method = '')
	{
		...
		return JHTML::_('select.genericlist', $methods, 'method', 'class="inputbox" size="3"', 'value', 'text', $default_method);
	}
