...
class HANDOUT_Utils
{
	...
	function array_diff_key()
	{
		...
		elseif (count($arrays) < 1) {
			trigger_error(JText::_('COM_HANDOUT_NOTARGGIVEN') . ", " . count($arrays) . " given, > 1 needed", E_USER_WARNING);
			return false;
		}
	...
	function checkDomainAuthorization()
	{
		...
		if (empty($from_host) || strcasecmp($this_host, $from_host) == 0 || strcasecmp('127.0.0.1', $from_host) == 0
				|| strcasecmp('localhost', $from_host) == 0) {
			$localhost = true;
		}
		else {
			$localhost = false;
...
class HANDOUT_Docs
{
	...
	function getDocsByUserAccess($catid = 0, $ordering = '', $direction = '', $limit = '', $limitstart = 0, $where = '')
	{
		...
		switch ($ordering) {
			case 'name':
				...
				break;
			case 'date':
				$ordering = 'd.docdate_published';
				break;
			case 'hits':
				$ordering = 'd.doccounter';
				break;
			default:
				$ordering = 'd.docname';
		}
	...
	function search(&$searchArray, $ordering = '', $cats = '', $columns = '', $options = array())
	{
		...
		$searchSortOrder = array('newest' => 'HANDOUT.doclastupdateon DDD', 'oldest' => 'HANDOUT.doclastupdateon AAA', 'popular' => 'HANDOUT.doccounter DDD',
				'alpha' => 'HANDOUT.docname AAA', 'category' => 'CAT.title AAA, SUB.title AAA, HANDOUT.docname AAA');
		...
		if ($invert) {
			$order = preg_replace($searchPattern, array('ASC', 'DESC'), $order);
		}
		else {
			$order = preg_replace($searchPattern, array('DESC', 'ASC'), $order);
		}
		...
		for ($r = 0; $r < count($rows); $r++) {
			$rows[$r]->section = @$options['section_prefix'] . HANDOUT_Docs::_handoutSearchSection($rows[$r]->catid, $cache, '/') . @$options['section_suffix'];
		}
		// FINAL SORT:
