...
class HandoutDocument extends JTable
{
	...
	function check()
	{
		...
		if ($this->docfilename == COM_HANDOUT_DOCUMENT_LINK AND $document_url = JRequest::getVar('document_url', false) AND parse_url($document_url)) {
		...
		if ($this->docowner == COM_HANDOUT_PERMIT_NOOWNER || $this->docowner == "") {
			$this->_error .= "\\n" . JText::_('COM_HANDOUT_ENTRY_OWNER');
		}

		if ($this->docmaintainedby == COM_HANDOUT_PERMIT_NOOWNER || $this->docmaintainedby == "") {
	...
	function authorCan($a = null)
	{
		return $this->_returnParam('author_can', '', $a);
	}

	function readerAssign($a = null)
	{
		return $this->_returnParam('reader_assign', '', $a);
	}

	function editorAssign($a = null)
	{
		return $this->_returnParam('editor_assign', '', $a);
	}
}
...
class HandoutCategory extends JTable
{
	...
	/**
	 * An alternative getInstance method because the function definitions are different between Joomla 1.5 and 1.6+.
	 *
	 * @param   interger  $id  The id of the category.
	 *
	 * @return  HandoutCategory
	 *
	 * @since   1.0
	 */
	public static function getInstance2($id = 0)
	{
		$db = &JFactory::getDBO();
...
class HandoutCodes extends JTable
{
	...
	function publish($cid, $publish)
	{
		...
		$cids = implode(',', $cid);
		$db->setQuery("UPDATE #__handout_codes SET published=" . (int) $publish . " \n WHERE id IN ($cids) ");
...
class HandoutLog extends JTable
{
	...
	function remove($cid) //removeLog
	{
		...
		if (count($cid)) {
			$cids = implode(',', $cid);
			$db->setQuery("DELETE FROM #__handout_log " . "\n WHERE id IN ($cids)");
