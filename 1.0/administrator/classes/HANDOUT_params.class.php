...
* @var string The type of setup file
*/
    function HandoutParameters( $text, $path='', $type='component' ) {
    ...
    function render( $name='params' ) {
        ...
        if (is_object( $this->_xmlElem )) {
            ...
            if (count( $element->childNodes ) < 1) {
                $html[] = "<tr><td colspan=\"2\">" . _NO_PARAMS . "</td></tr>";
            }
            return implode( "\n", $html );
        } else {
...
*/
    function renderParam( &$param, $control_name='params' ) {
        ...
        if ($result[0] == '@spacer') {
            ...
            $result[0] = HANDOUT_Utils::mosToolTip( addslashes( $description ), addslashes( $result[0] ), '', '', $result[0], '#', 0 );
        }
        ...
        if ( $description ) {
            $result[2] = HANDOUT_Utils::mosToolTip( $description, $result[0] );
            $result[2] = '';
        } else {
            $result[2] = '';
        }
    ...
    */
    function _form_radio( $name, $value, &$node, $control_name ) {
        ...
        return JHTML::_('select.radiolist', $options, ''. $control_name .'['. $name .']', '','value', 'text', $value );
    }
    /**
    * @param string The name of the form element
    * @param string The value of the element
...
* @return string
*/
function HandoutParseParams( $txt ) {
    return HandoutParameters::parse( $txt );
}
...
* @package HANDOUT_1.0
*/
class HandoutParametersHandler extends HandoutParameters
{
	/**
	* Constructor
    * Joomla! 1.0 style
	...
	* @var string The type of setup file
	*/
	function HandoutParametersHandler( $text, $path='', $type='component' ) {
		parent::HandoutParameters($text, $path, $type);
	}
	...
	*/
	...
	function _renderStandard(&$element, $name)
	{
		...
		foreach ($element->childNodes as $param)
		{
			...
			if($type == 'heading') {
				$html[] = '<td width="100%" class="title" colspan="3">' . $result[0] . '</td>';
			} else {
				$html[] = '<td width="20%" style="text-align:left;">' . $result[0] . '</td>';
                $html[] = '<td width="5%" style="text-align:left;">' . $result[2] . "</td>";
				$html[] = '<td width="100%" style="text-align:left;">' . $result[1] . '</td>';
	...
	*/
	function renderParam( &$param, $control_name='params' )
	{
		...
		if ( $description ) {
			$result[2] = HANDOUT_Utils::mosToolTip( $description, $name );
		} else {
			$result[2] = '';
		}
