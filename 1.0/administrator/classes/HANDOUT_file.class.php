 ...
 * @version 	$Id: handout_file.class.php
 * @package 	Handout
...
require_once($_HANDOUT->getPath('includes', 'defines'));	// define JText::_('COM_HANDOUT_VALIDATE_')xxxx
...
class HANDOUT_FileUpload
{
	...
	*/
    function uploadLINK($url,  $validate=COM_HANDOUT_VALIDATE_ALL )
    {
        ...
        if( $validate &
			COM_HANDOUT_VALIDATE_PROTO  && ! $this->validateProtocol( $parsedurl['scheme'])){
            	return false;
        }
	...
	*/
    ...
    function validateName(&$name)
    {
		...
		if( ($this->fname_reject && preg_match( "/^(" . $this->fname_reject . ")$/i" , $name ) )
            OR preg_match( "/^(" . COM_HANDOUT_FNAME_REJECT . ")$/i" , $name )){
	...
	*/
	function getFiles($match_filter=null, $ignore_filter=null, $filter=null)
	{
		...
		while ( $file = @readdir($handle) ) {
            ...
            if ( preg_match("/^".COM_HANDOUT_FNAME_REJECT."^/",$file) ) continue;
