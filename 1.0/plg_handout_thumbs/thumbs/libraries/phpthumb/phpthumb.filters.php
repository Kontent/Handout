...
//exit;
	...
	function WatermarkText(&$gdimg, $text, $size, $alignment, $hex_color='000000', $ttffont='', $opacity=100, $margin=5, $angle=0, $bg_color=false, $bg_opacity=0, $fillextend='') {
		...
		if (preg_match('^([0-9\\.\\-]*)x([0-9\\.\\-]*)(@[LCR])?$', $alignment, $matches)) {
		...
		if (@is_readable($ttffont) && is_file($ttffont)) {
			...
			if ($alignment == '*') {
				...
				$x_TL = preg_match('x', $fillextend) ?               0 : min($x1, $x2);
				$y_TL = preg_match('y', $fillextend) ?               0 : min($y1, $y2);
				$x_BR = preg_match('x', $fillextend) ? ImageSX($gdimg) : max($x1, $x2);
				$y_BR = preg_match('y', $fillextend) ? ImageSY($gdimg) : max($y1, $y2);
