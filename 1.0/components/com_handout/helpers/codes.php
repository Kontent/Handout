...
class CodesHelper {
	...
	function processCode($docid) {
		...
		$code = CodesHelper::getCode($docid);

		if (!$code || $code->name != $codeVal) {
			HandoutHelper::_returnTo ( 'doc_code', JText::_('COM_HANDOUT_CODE_INVALID'), $docid) ;
		}
		else {
			...
			switch ($code->usage) {
				case '0': //single-use
					...
					if ( CodesHelper::getUsage($codeVal, $docid) > 0) {
						HandoutHelper::_returnTo ( 'doc_code', JText::_('COM_HANDOUT_CODE_LIMIT_EXCEED'), $docid);
					}
					break;
				case '1': //unlimited
					//do nothing
			...
			switch ($code->user) {
				...
				case '1': //registered
					...
					if ($handoutUser->userid == 0) {
						//not logged in
						HandoutHelper::_returnTo ( 'doc_code', JText::_('COM_HANDOUT_CODE_NOLOGIN'), $docid);
					}
					else {
				...
				case '2': // email required 
					//TODO: need to do sanity check on email before proceeding
					
					if (!$email || !preg_match("/^[a-z0-9]+([_\\.-][a-z0-9]+)*@([a-z0-9]+([\.-][a-z0-9]+)*)+\\.[a-z]{2,}$/i", $email)) {
						HandoutHelper::_returnTo ( 'doc_code', JText::_('COM_HANDOUT_CODE_INVALID_EMAIL'), $docid);						
					}
