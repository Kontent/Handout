...
{      public $_HANDOUT=null;
	...
	function getDocument($id) {
		...
		$bot->trigger ();
		if ($bot->getError ()) {
		$this->_returnTo ( 'cat_view', $bot->getErrorMsg () );
		}
	...
	function getDocumentList($catid) {
		...
		$links = array ();
		$links ['name'] = $this->_taskLink ( 'cat_view', $catid, array_merge ( $params, array ('order' => 'name', 'dir' => $direction ) ) );
		$links ['date'] = $this->_taskLink ( 'cat_view', $catid, array_merge ( $params, array ('order' => 'date', 'dir' => $direction ) ) );
		$links ['hits'] = $this->_taskLink ( 'cat_view', $catid, array_merge ( $params, array ('order' => 'hits', 'dir' => $direction ) ) );

		if ($direction == 'ASC') {
			$links ['dir'] = $this->_taskLink ( 'cat_view', $catid, array_merge ( $params, array ('order' => $ordering, 'dir' => 'DESC' ) ) );
		} else {
			$links ['dir'] = $this->_taskLink ( 'cat_view', $catid, array_merge ( $params, array ('order' => $ordering, 'dir' => 'ASC' ) ) );
		}
	...
	function getEditDocumentForm($uid, $filename = null, $catid = 0) {
		...
		$lists ['docfilename'] = $this->filesSelectList ( $doc );
		...
		$lists ['licenses'] = $this->licenseSelectList ( $doc );
		$lists ['licenses_display'] = $this->licenseDisplayList ( $doc );
	 ...
	 * ------------------------------ */
		...
		if ($prebot->getError ()) {
			$this->_returnTo ( 'cat_view', $prebot->getErrorMsg () );
		}
	...
	function getMoveDocumentCategories($gid) {
	...
	function publishDocument($cid, $publish = 1) {
		...
		$this->_returnTo ( 'cat_view', '', $doc->catid );
	}
	 ...
	 * ------------------------------ */
		...
		if ($postbot->getError ()) {
			...
			$logbot->trigger ();
			$this->_returnTo ( 'cat_view', $postbot->getErrorMsg () );
		}
		...
		$this->_returnTo ( 'cat_view', JText::_('COM_HANDOUT_PROBLEM_SAVING_DOCUMENT') );
	}
	...
	function cancelDocument($gid) {
		...
		if (! $uid) {
			$this->_returnTo ( 'cat_view', JText::_('COM_HANDOUT_OP_CANCELED') );
		}
		...
		if ($doc->cancel ()) {
			$this->_returnTo ( 'cat_view', JText::_('COM_HANDOUT_OP_CANCELED'), $doc->catid );
		}
	...
	function resetDocument($uid) {
		...
		$this->_returnTo ( 'cat_view', JText::_('COM_HANDOUT_RESET_COUNTER'), $doc->catid );
	}
	...
	function deleteDocument($uid) {
		...
		$this->_returnTo ( 'cat_view', JText::_('COM_HANDOUT_DOCDELETED'), $doc->catid );
	}
	 ...
	 */
		...
		if (! $doc->id) {
			...
			if (count ( $options ) < 2) {
				//$this->_returnTo('upload', JText::_('COM_HANDOUT_YOU_MUST_UPLOAD'));
